...
import ollama
from pa import generate_resume_pdf, generate_cover_letter_pdf

st.set_page_config(page_title="AI Resume & Cover Letter Builder", page_icon="📝")
st.title("✨ AI Resume & Cover Letter Builder (Classic Version)")
...
# --- Generate Resume ---
if generate_resume:
    st.info("✨ Generating your resume... please wait...")
    model = "llama3"
...
"""
    ...
    sections = [s.strip() for s in response.split('###SECTION###')]
    while len(sections) < 7:
        sections.append("")
    profile, education, projects_ai, skills_ai, soft_skills_ai, extras_ai = sections[1:7]
...
if generate_cover:
    st.info("✨ Generating your cover letter...")
    model = "llama3"
...
"""
    ...
    cover_text = ollama.chat(model=model, messages=[{'role':'user','content': cover_prompt}])['message']['content'].strip()

    cover_pdf = generate_cover_letter_pdf(name, email, phone, linkedin, cover_text)
